...
6. Run the front matter validation script to set the correct tags and validate the article:
   ```
   /app/bin/validate-frontmatter.sh /out/article.md
   ```
   This script programmatically sets the `tags` field with the correct tag-safe values for the repository name and model name, validates the YAML front matter, and asserts that all required fields (`layout`, `title`, `date`, `tags`) are present and correct.
   If the script reports a failure, review the error output, fix the article accordingly, and re-run the script until it passes.
